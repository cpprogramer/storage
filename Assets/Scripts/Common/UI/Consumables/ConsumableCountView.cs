...
namespace JigsawPuzzles.UI.View
{
    public sealed class ConsumableCountView : BaseConsumableView
    {
        ...
        {
            string text = string.Format( _format, count );
            if ( text != _textCount.text )
            {
                GameObject inst = Instantiate( _coinsTemplate, _coinsTemplate.transform.parent, true );
                timeline.Add( () =>
                {
                    inst.SetActive( true );
                ...
                timeline.Add( () => { inst.transform.DOLocalMove( Vector3.zero, _moveDuration ); } );
